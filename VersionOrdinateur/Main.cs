...
public class Main : MonoBehaviour
{
    ...
    public bool coutureActive = false;
    ...
    public Button Cloth_button, Modify_button, Sew_button, Patron_button;
    public bool modify_clicked = false, sew_clicked = false, cloth_clicked = false, patron_clicked;
    ...
    void Start()
    {
        ...
        StartCoroutine(ShowTemporaryMessage("Appuyez sur R pour créer un ruban et faites un clic gauche pour arrêter."));
    }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.R) && MeshCreation == null && !modeCloth && !coutureActive && !modeModify && !patron_clicked)
        {
            NewMesh();
        ...
        if (Input.GetMouseButtonUp(0) && !coutureActive)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hit))
            {
                if (rubans.Contains(hit.collider.gameObject))
                {
                    ...
                    Debug.Log("Ruban ou patron sélectionné : " + rubanSelectionne.name);
                ...
                if (patronManager.patrons.Contains(hit.collider.gameObject))
                {
                    rubanSelectionne = hit.collider.gameObject;
                    Debug.Log("Ruban ou patron sélectionné : " + rubanSelectionne.name);
    ...
    public void ModifyButton()
    {
        ...
        if (modeModify)
        {
            if (rubanSelectionne == null)
            {
                StartCoroutine(ShowTemporaryMessage("Selectionnez un ruban ou un patron à modifier !"));
            }
    ...
    public void SewButton()
    {
        ...
        if (sew_clicked)
            StartCoroutine(ShowTemporaryMessage("Selectionnez deux rubans puis appuyez sur la touche I pour coudre."));
