...
public class DemoV0 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (coutureActive)
        {
            ...
            if (rubanSelectionne != null)
            {
                ...
                {
                    GameObject rubanA = rubanSelectionne;
                    ...
                    {
                        if (rubans[j] == rubanSelectionne) continue;
    ...
    void CloseRuban()
    {
        // Liste de tous les rubans proches
        HashSet<GameObject> rubansProches = new HashSet<GameObject>();

        // Étape 1 : détection
        for (int i = 0; i < rubans.Count; i++)
        {
            var rubanA = rubans[i];
            if (rubanA == null) continue;

            MeshFilter filterA = rubanA.GetComponent<MeshFilter>();
            if (filterA == null) continue;
            Mesh meshA = filterA.mesh;
            if (meshA == null || meshA.vertices.Length < 2) continue;

            Vector3[] verticesA = meshA.vertices;
            Vector3 pointA = rubanA.transform.TransformPoint(verticesA[verticesA.Length - 2]);

            for (int j = i + 1; j < rubans.Count; j++)
            {
                var rubanB = rubans[j];
                if (rubanB == null) continue;

                MeshFilter filterB = rubanB.GetComponent<MeshFilter>();
                if (filterB == null) continue;
                Mesh meshB = filterB.mesh;
                if (meshB == null || meshB.vertices.Length < 2) continue;
                ...
                if (distance < 1.0f)
                {
                    rubansProches.Add(rubanA);
                    rubansProches.Add(rubanB);
                }
            }
        }
        ...
        foreach (var ruban in rubans)
        {
            if (ruban == null) continue;
            Renderer rend = ruban.GetComponent<Renderer>();
            if (!rend) continue;

            if (rubansProches.Contains(ruban))
...
// Coudre le ruban sélectionné en tant que premier ruban
// Les couleurs doivent se faire à nouveau après couture
